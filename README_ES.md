...
flowchart TD
    ...
    subgraph Source["1. Ingesta y Web Scraping"]
        direction LR
        A["API Portal Inmobiliario / Metrocuadrado"]:::external
        B["Archivos Parquet Crudos (Bronze/)"]:::external
        A -->|"Python Scraper"| B
    end

    subgraph DockerEnv["2. Entorno de Procesamiento Aislado (Docker Desktop)"]
        direction TB

        subgraph Pipeline["Servicio data-pipeline (Python 3.11)"]
            direction TB

            subgraph DBT_Layer["Transformación de Datos y Modelo Estrella (dbt + DuckDB)"]
                direction LR
                C["Capa Staging<br/>main_staging.staging_renting"]:::dbt
                D["Capa Silver<br/>main_intermediate.silver"]:::dbt
                E["Capa Gold - Tabla de Hechos<br/>main_mart.fact_renting"]:::dbt
                ...
                G{"Pruebas de Calidad de Datos<br/>(schema.yml + integer_checking)"}:::dbt
    ...
    subgraph Downstream["3. Visualización y CI/CD"]
        direction LR

        subgraph PowerBI["Dashboard en Power BI"]
            direction TB
            H["Dashboard de Analítica Inmobiliaria"]:::pbi
            H1["KPIs Principales del Negocio:<br/>Total Inmuebles, Precio Mediano, Precio/m² Mediano, Área Mediana, Días en Mercado Mediano"]:::pbi
            H2["Dinámicas de Mercado y Rankings:<br/>Precio vs. Días en Mercado, Profundidad de Mercado, Barrios Asequibles/Exclusivos"]:::pbi
            H3["Segmentaciones y Filtros:<br/>Tipo de Inmueble, Ciudad, Barrio, Estrato, Precio y Estado"]:::pbi
        ...
        J["Ruff (Linting) y dbt test (Validación)"]:::external

        DB -->|"Conector DuckDB"| H
        I -->|"Disparadores Automáticos"| J
        J -.-> DockerEnv
    end
